    ...
    recipes.forEach((recipe) => {
        ...
        card.innerHTML = `
            ...
            recipe.label
                    ...
                    <div class="p-4">
                        <h4 class="text-white font-semibold text-lg mb-2">${escapeHtml(
            recipe.label
        )}</h4>
                        <p class="text-gray-300 text-sm mb-3">Approx: ${recipe.calories
            } kcal • ${recipe.protein}g P • ${recipe.carbs}g C • ${recipe.fat
            }g F</p>
                        <div class="flex gap-2">
                            ...
                            <a href="${recipe.url
            }" target="_blank" class="flex-1 text-center underline text-sm text-white/90 py-2 rounded-lg border border-white/10">View</a>
                        </div>
                    </div>
                `;
...
const mockFoodDB = {
    ...
    "orange juice": { calories: 45, protein: 0.7, carbs: 11, fat: 0.2 },
    smoothie: { calories: 130, protein: 3, carbs: 30, fat: 1 },

    // Sweets
    chocolate: { calories: 546, protein: 4.9, carbs: 61, fat: 31 },
    cake: { calories: 257, protein: 3.5, carbs: 51, fat: 8.9 },
    cookies: { calories: 502, protein: 5.8, carbs: 64, fat: 24 },
};
...
function addFoodItem(item) {
    ...
    // Visual feedback
    document
        .getElementById("consumed-calories")
...
function renderFoodLog() {
    ...
    if (foodLog.length === 0) {
        logEl.innerHTML =
            '<p class="text-gray-400 text-center py-8">No food items logged yet. Start by adding some food above!</p>';
        return;
    }
        ...
        .forEach((f, idx) => {
            ...
            item.innerHTML = `
                ...
                f.name
                        ...
                        <div class="text-gray-300 text-sm">${f.quantity ? f.quantity + "g" : ""
                } • ${f.calories} kcal • ${f.protein}g P</div>
                    </div>
...
function removeFoodItem(index) {
    ...
    // Subtract nutrition
    consumedNutrition.calories = Math.max(
        0,
        consumedNutrition.calories - Number(f.calories)
    );
    consumedNutrition.protein = Math.max(
        0,
        consumedNutrition.protein - Number(f.protein)
    );
    consumedNutrition.carbs = Math.max(
        0,
        consumedNutrition.carbs - Number(f.carbs)
    );
...
function updateNutritionProgress() {
    ...
    document.getElementById("consumed-calories").textContent =
        consumedNutrition.calories;
    document.getElementById("consumed-protein").textContent = `${Math.round(
        consumedNutrition.protein
    )}g`;
    document.getElementById("consumed-carbs").textContent = `${Math.round(
        consumedNutrition.carbs
    )}g`;
    document.getElementById("consumed-fat").textContent = `${Math.round(
    ...
    const calPercent = Math.min(
        100,
        Math.round((consumedNutrition.calories / calGoal) * 100)
    );
    document.getElementById(
        "calorie-progress"
    ).textContent = `${consumedNutrition.calories} / ${calGoal} kcal`;
    document.getElementById(
    ...
    if (proteinEl && carbsEl && fatEl) {
        ...
        const pPerc = proteinGoal
            ? Math.min(
                100,
                Math.round((consumedNutrition.protein / proteinGoal) * 100)
            )
            : 0;
        const cPerc = carbsGoal
            ? Math.min(100, Math.round((consumedNutrition.carbs / carbsGoal) * 100))
            : 0;
        const fPerc = fatGoal
            ? Math.min(100, Math.round((consumedNutrition.fat / fatGoal) * 100))
            : 0;
...
async function searchRecipes(query = null) {
    const searchTerm =
        query || document.getElementById("recipe-search").value.trim();
    if (!searchTerm) {
...
document.addEventListener("click", function (e) {
    if (
        ...
        !e.target.classList.contains("no-feedback")
    ) {
...
function getNutritionalInsight(consumedCalories, goalCalories) {
    ...
    if (percentage < 70) {
        return {
            message:
                "You're below your calorie goal. Consider adding a healthy snack.",
            color: "text-yellow-400",
        };
    } else if (percentage > 110) {
        return {
            message: "You've exceeded your calorie goal. Great job staying active!",
            color: "text-red-400",
        };
    } else {
        return {
